...
- [x] Install SQLAlchemy ORM dependency: `pip install sqlalchemy pgvector`.
- [x] Create `src/kb_web/models_orm.py` and declare ORM mapping classes using the declarative base pattern.
- [x] Map active SQLite tables to SQLAlchemy classes, specifying model field types, indexes, and relationships:
...
- [x] Setup view mappings (`vault_master`, `repo_master`, `valid_repo_files`) using the declarative ORM read-only format.

### 2. Database Session & Dialect configs (Sub-Issue #44)
- [x] Add the `DATABASE_URL` string setting inside `src/kb_web/config.py`.
- [x] Refactor connection pool engine mappings in `src/kb_web/base.py` to:
...
- [x] Map embeddings columns inside `models_orm.py` (`ArticleEmbedding`, `VideoEmbedding`, `ChunkEmbedding`) using the `Vector` field type:
  ```python
  from pgvector.sqlalchemy import Vector
  embedding = Column(Vector(1536))  # 1536 size matches EmbeddingGemma / OpenAI dimensions
  ```
- [x] Ensure database startup triggers `CREATE EXTENSION IF NOT EXISTS vector;` on PostgreSQL connection instances.
- [x] Refactor similarity search operations inside `src/kb_web/utils.py` and pages/RAG routers:
