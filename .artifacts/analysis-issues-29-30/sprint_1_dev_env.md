...
- [ ] Define workspace config in `.devcontainer/devcontainer.json` mapping environment variables and launching the postgres docker-compose stack.
...
- [ ] Write the first baseline migration script under `migrations/versions/` representing the base database layout.
- [ ] Write `scripts/deploy_migrations.py` utilizing the Alembic programmatic API (`alembic.config.Config`, `alembic.command.upgrade`) to run pending migrations on server startup.
