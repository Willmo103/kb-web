    ...
    class StandaloneApplication:
        ...
        def run(self):
            raise NotImplementedError(
                "Gunicorn is not supported on Windows or non-POSIX platforms."
            )


def run_server(
    app_uri: str, host: str, port: int, workers: int = None, reload: bool = False
) -> None:
    """
    ...
    if sys.platform == "win32" or reload or not GUNICORN_AVAILABLE:
        ...
        print(
            f"Starting server using Uvicorn on http://{host}:{port} (reload={reload})"
        )
        ...
        print(
            f"Starting production server using Gunicorn on http://{host}:{port} (workers={workers})"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the production Gunicorn/Uvicorn server."
    )
    ...
    parser.add_argument(
        "--workers", type=int, default=None, help="Number of Gunicorn workers"
    )
    parser.add_argument(
        "--reload", action="store_true", help="Use hot-reload (Uvicorn only)"
    )
    parser.add_argument(
        "--app-uri", default="kb_web.server:app", help="ASGI Application URI"
    )
