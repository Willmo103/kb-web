...
def sync_collection_to_qdrant(db, collection_id: int) -> tuple[bool, str]:
    ...
    # Gather points
    items = list(
        db["collection_items"].rows_where("collection_id = ?", [collection_id])
    )
    ...
    for item in items:
        ...
        chunks = list(
            db["chunk_embeddings"].rows_where(
                "source_type = ? AND source_id = ?", [source_type, source_id]
            )
        )
        ...
        for chunk in chunks:
            ...
            pt_id = str(
                uuid.uuid5(uuid.NAMESPACE_URL, f"{source_type}:{source_id}:{chunk_num}")
            )
            ...
            points.append({"id": pt_id, "vector": vector, "payload": payload})
    ...
    with httpx.Client(timeout=client_timeout) as client:
        try:
            ...
            return (
                False,
                f"Qdrant connection error. Saved sync locally. Details: {conn_err}",
            )
        ...
        try:
            ...
            return (
                False,
                f"Qdrant point upload failed. Saved sync locally. Details: {upsert_err}",
            )
    ...
    return (
        True,
        f"Successfully synced {len(points)} points to Qdrant collection '{col_name}'.",
    )
...
def save_collection_items(
    ...
    try:
        ...
        return JSONResponse(
            content={
                "status": "success",
                "message": f"Successfully updated {len(urls)} collection items.",
            }
        )
    except Exception as e:
        return JSONResponse(
...
@router.post(
    "/collections/view/{collection_id}/save-settings",
...
@router.post(
    "/collections/view/{collection_id}/sync", dependencies=[Depends(verify_auth)]
)
...
async def update_page_collection(
    request: Request, url: str = Form(...), collection_id: Optional[str] = Form(None)
) -> RedirectResponse:
    ...
    try:
        ...
        db["collection_items"].delete_where(
            "source_id = ? AND collection_id != ?", [url, general_id]
        )
        db.conn.commit()
...
def remove_page_from_collection(
    ...
    try:
        ...
        if collection_id:
            ...
            db["collection_items"].delete_where(
                "source_id = ? AND collection_id != 1", [url]
            )
        db.conn.commit()
    except Exception as e:
...
""".replace("{general_id}", str(general_id))
        ...
        for idx, page in enumerate(pages):
            ...
            try:
                if list(
                    db["collection_items"].rows_where(
                        "collection_id = ? AND source_id = ?", [general_id, url]
                    )
                ):
                    is_present = True
            except Exception:
            ...
            try:
                ...
                system_instructions = compile_taxonomy_system_prompt(
                    collection_general, db
                )
            except Exception:
                system_instructions = (
            ...
            try:
                ...
                # Fallback on failure
                warn_msg = (
                    f"Warning: Ollama prompt failed, using default paths: {llm_err}"
                )
...
def suggest_collections() -> JSONResponse:
    ...
    try:
        ...
        for sug in suggestions_data.get("suggestions", []):
            ...
            if sug_pages:
                formatted_suggestions.append({"title": sug_title, "pages": sug_pages})
...
def compile_taxonomy_system_prompt(collection, db) -> str:
    ...
    if "collection_items" in db.table_names():
        ...
        for item in items:
            path = (
                item.get("taxonomy_path") or f"/uncategorized/{item['source_id'][-20:]}"
            )
...
def compile_system_prompt(collection, db) -> str:
    prompt = (
        collection.get("rag_system_prompt")
            ...
            .insert(
                {
                    ...
                    "title": title,
                    "content": "# "
                    + title.replace(".md", "").replace("_", " ").title()
                    + "\n\nStart writing here...",
                    "taxonomy_path": taxonomy_path,
                    "created_at": now,
                    "updated_at": now,
...
def update_collection_item_note(
    ...
    try:
        ...
        db["collection_items"].update(
            item_id, {"item_note": item_note, "taxonomy_path": taxonomy_path}
        )
        db.conn.commit()
        return JSONResponse(
            content={
...
def collection_agent_chat(
    ...
    try:
        client = _get_ollama_client()
        response = client.chat(
            model=config.ollama_model, messages=messages, think=False
        )
...
def view_collection_editor(request: Request, collection_id: int) -> HTMLResponse:
    ...
    if "collection_notes" in db.table_names():
        try:
            notes = list(
                db["collection_notes"].rows_where(
                    "collection_id = ?", [collection_id], order_by="title ASC"
                )
            )
        except Exception as e:
